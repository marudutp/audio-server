...
import { dirname, join } from 'path';  // 🔥 PERBAIKAN: tambah 'join'
import fs from 'fs';
import os from 'os';
...
interface AudioUser {
    ...
    position?: { x: number; y: number; z: number }; // Untuk spasial audio
}
...
io.on('connection', (socket: any) => {
    ...
    socket.on('register_audio', (data: { uid: string, displayName: string, role: string, movementSocketId?: string }) => {
        ...
        // Kirim daftar audio users yang sudah ada
    ...
    // ============================================
    // WEBRTC SIGNALING (Audio-specific)
    // ============================================
...
// SERVER START
// ============================================
// const AUDIO_PORT = process.env.AUDIO_PORT || 8081;
